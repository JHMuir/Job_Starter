def generate_job_embeddings(encoder, X):
    ...
    embeddings = encoder.???(???, verbose=0)
    
    return embeddings


def encode_user_input(user_description, user_skills, user_category, user_summary, user_title, encoders):
    """
    ...
    X_desc = encoders['???'].transform([user_description])
    
    # Encode skills using the SAME MultiLabelBinarizer
    # Unknown skills are simply ignored (not in the learned vocabulary)
    X_skills = encoders['???'].???([user_skills])
    
    # Encode category using the SAME OneHotEncoder
    # Must be a DataFrame with the same column name
    X_cat = encoders['???'].???(user_category)
    
    X_summary = encoders['???'].???([user_summary])
    
    x_title = encoders['???'].???([user_title])
    
    # Combine in the SAME order as training
    X_user = ???([X_desc, X_skills, X_cat, X_summary, x_title])
    
    return X_user


def recommend_jobs(user_description, user_skills, user_category, 
    ...
    X_user = encode_user_input(user_description, user_skills, user_category, user_summary, user_title, encoders)
    
    # Step 2: Generate user embedding
    X_user_dense = X_user.toarray().astype('float32')
    user_embedding = encoder.???(X_user_dense, verbose=0)
